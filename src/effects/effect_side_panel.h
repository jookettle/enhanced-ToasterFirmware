...
public:
  ...
  virtual void process(Display& display) {
    uint8_t r = _personalColor[Effect::_personalIndex][PC_SIDE][0];
    uint8_t g = _personalColor[Effect::_personalIndex][PC_SIDE][1];
    uint8_t b = _personalColor[Effect::_personalIndex][PC_SIDE][2];

    _colorFunc(0, 0, r, g, b, 255, PC_SIDE);
    display.fillHalf(0, r, g, b);

    r = _personalColor[Effect::_personalIndex][PC_SIDE + PC_MAX][0];
    g = _personalColor[Effect::_personalIndex][PC_SIDE + PC_MAX][1];
    b = _personalColor[Effect::_personalIndex][PC_SIDE + PC_MAX][2];
