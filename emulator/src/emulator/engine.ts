...
import type { ESP32Image } from './binary-parser';
import { XtensaCPU } from './cpu';
import { ESP32HAL } from './hal';

export class EmulatorEngine {
  private cpu: XtensaCPU;
  private hal: ESP32HAL;
  private image: ESP32Image | null = null;
  private displayCallback: ((buffer: Uint8Array) => void) | null = null;
  private frameUpdateInterval: ReturnType<typeof setInterval> | null = null;
  private overlayInterval: ReturnType<typeof setInterval> | null = null;
  private overlayState = false;

  constructor() {
    this.cpu = new XtensaCPU();
    this.hal = new ESP32HAL((msg: string) => {
      this.cpu.onLog(msg);
    });
    this.cpu.hal = this.hal;
  }

  async loadBinary(buffer: ArrayBuffer, onLog: (m: string) => void) {
    try {
  ...
  start(onLog: (m: string) => void, onDisplay: (b: Uint8Array) => void) {
